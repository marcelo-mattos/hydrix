...
namespace Hydrix.UnitTests.Schemas
{
    ...
    /// valid and invalid states. It also validates delegate-based external validation integration and checks the
    /// results of the validation process.</remarks>
    public class DatabaseEntityTests
    {
        ...
        /// <summary>
        /// Executes external validation rules for <see cref="TestEntity"/> instances.
        /// </summary>
        /// <param name="entity">The entity instance to validate.</param>
        /// <returns>A collection of validation results generated by the external validation rules.</returns>
        private static IEnumerable<ValidationResult> ValidateTestEntityExternally(TestEntity entity)
        {
            if (string.IsNullOrWhiteSpace(entity.Name))
                yield return new ValidationResult(
                    "Name must not be empty.",
                    new[] { nameof(TestEntity.Name) });

            if (entity.Value < 1 || entity.Value > 10)
                yield return new ValidationResult(
                    "Value must be between 1 and 10.",
                    new[] { nameof(TestEntity.Value) });
        }
        ...
        /// <summary>
        /// Verifies that a TestEntity instance is considered valid by the IsValid method when evaluated with an
        /// external validator delegate.
        /// </summary>
        /// <remarks>This test ensures that the IsValid method returns true and produces no validation
        /// errors when the entity meets all validation criteria defined by external validation rules.</remarks>
        [Fact]
        public void IsValid_WithExternalValidator_ReturnsTrue_WhenEntityIsValid()
        {
            var entity = new TestEntity { Name = "Valid", Value = 5 };
            var result = entity.IsValid<TestEntity>(
                out List<ValidationResult> errors,
                ValidateTestEntityExternally);
            Assert.True(result);
            Assert.Empty(errors);
        }
        ...
        /// satisfy external validation rules.
        /// </summary>
        ...
        /// external validator's effectiveness in identifying invalid input.</remarks>
        [Fact]
        public void IsValid_WithExternalValidator_ReturnsFalse_WhenEntityIsInvalid()
        {
        ...
        /// when the validator parameter is null, confirming that external validation is not executed in this
        /// scenario.</remarks>
        [Fact]
        public void IsValidT_DoesNotRunExternalValidation_WhenValidatorIsNull()
        {
        ...
        /// <summary>
        /// Verifies that the IsValid&lt;T&gt; method throws when the generic type parameter does not match the entity
        /// runtime type.
        /// </summary>
        /// <remarks>This test validates the explicit cast contract used by IsValid&lt;T&gt; when an external
        /// validator is provided.</remarks>
        [Fact]
        public void IsValidT_ThrowsInvalidCastException_WhenEntityIsNotOfTypeT()
        {
            var entity = new DatabaseEntityTests.TestEntity { Name = "Valid", Value = 5 };
