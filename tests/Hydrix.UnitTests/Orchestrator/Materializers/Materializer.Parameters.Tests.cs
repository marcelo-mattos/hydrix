...
{
    /// <summary>
    /// Contains unit tests for the Materializer class, verifying parameter handling and formatting behaviors for SQL
    ...
    /// related interfaces to isolate and validate Materializer logic without requiring a real database
    /// connection.</remarks>
    public partial class MaterializerTests
    {
        ...
        public void FormatParameterValue_HandlesCommonTypes(object value, string expected)
        {
            if (value is string s && DateTime.TryParse(s, out var dt))
                Assert.Equal($"'{dt:yyyy-MM-dd HH:mm:ss.fff}'", TestMaterializerParameter.CallFormatParameterValue(dt));
            else
                Assert.Equal(expected, TestMaterializerParameter.CallFormatParameterValue(value));
        }
        ...
        public void FormatParameterValue_Guid()
        {
            ...
            Assert.Equal($"'{guid}'", TestMaterializerParameter.CallFormatParameterValue(guid));
        }
        ...
        {
            Assert.Equal("NULL", TestMaterializerParameter.CallFormatParameterValue(DBNull.Value));
        }
    }
}