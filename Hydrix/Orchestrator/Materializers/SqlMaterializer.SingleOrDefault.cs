...
namespace Hydrix.Orchestrator.Materializers
{
        ...
        Contract.ISqlMaterializer
    {
        ...
        /// Then parse its result into a ISqlEntity object returning the processed data to the requester.
        /// </summary>
        ...
        public async Task<TEntity> SingleOrDefaultAsync<TEntity>(
            ...
            where TEntity : ISqlEntity, new()
        {
            var result = await this.QueryAsync<TEntity>(
                sql,
                parameters,
                cancellationToken)
        ...
        public async Task<TEntity> SingleOrDefaultAsync<TEntity>(
            ...
            CancellationToken cancellationToken = default)
            where TEntity : ISqlEntity, new()
            => await this.SingleOrDefaultAsync<TEntity>(
                sql,
                (object)null,
                cancellationToken)
            .ConfigureAwait(false);
        ...
        public async Task<TEntity> SingleOrDefaultAsync<TEntity, TDataParameterDriver>(
            ...
            where TDataParameterDriver : IDataParameter, new()
        {
            var result = await this.QueryAsync<TEntity, TDataParameterDriver>(
                sqlProcedure,
                cancellationToken)
                .ConfigureAwait(false);
